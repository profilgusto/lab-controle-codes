...
e o erro maximo de cada grau - reproduzindo a comparacao que motivou o uso de
um polinomio em vez de uma reta em `conversoes.py` - e devolve os coeficientes
do grau escolhido, prontos para colar em `LT_COEFS_H_DE_CONTAS`.
...
def desenha(h, contas, ajustes, arquivo):
    ...
    for grau, (coefs, rmse, _erro_max) in ajustes.items():
        ax.plot(c_fino, np.polyval(coefs, c_fino), '-',
                label=f'grau {grau} (RMSE = {rmse:.2f} mm)')
...
def main():
    ...
    print()
    print(f'{"grau":>4}   {"RMSE [mm]":>10}   {"erro max [mm]":>14}')

    ajustes = {}
    for grau in (1, 2, 3):
        coefs, rmse, erro_max = ajusta_grau(h, contas, grau)
        ajustes[grau] = (coefs, rmse, erro_max)
        print(f'{grau:>4}   {rmse:>10.2f}   {erro_max:>14.2f}')

    coefs, rmse, erro_max = ajustes[args.grau]
    print()
